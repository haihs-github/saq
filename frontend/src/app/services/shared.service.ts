...
export class SharedService {
  ...
  private BorrowReturnSlipDetailId = new BehaviorSubject<any>(null)
  private id = new BehaviorSubject<string>("")
  private typeAction = new BehaviorSubject<string>("")
  getThietBi = this.thietBi.asObservable()
  getTypeAction = this.typeAction.asObservable()
  getSearch = this.search.asObservable()
  getId = this.id.asObservable()
  getBorrowReturnSlipDetailId = this.BorrowReturnSlipDetailId.asObservable()

  setThietBi(data: any) {
    this.thietBi.next(data)
  }
  setTypeAction(data: any) {
    this.typeAction.next(data)
  }
  setBorrowReturnSlipDetailId(id: string) {
    this.BorrowReturnSlipDetailId.next(id)
  }

  setSearch(data: any) {
    this.search.next(data)
  }
  setId(id: string,typeEquipment: string ) {
    this.id.next(id+"|"+typeEquipment)
  }
  getDMY(){
